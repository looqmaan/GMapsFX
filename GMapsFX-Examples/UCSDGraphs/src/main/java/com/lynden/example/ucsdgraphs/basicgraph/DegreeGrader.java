...
public class DegreeGrader {
     ...
     * @param desc A description of the graph
     * @param desc The node to start from
     * @param desc A list containing the correct answer
        ...
        try {
            ...
            reader = new BufferedReader(
                new InputStreamReader(
                    getClass().getResourceAsStream(classPathPrefix + file)
                )
            );
            ...
            while ((nextLine = reader.readLine()) != null) {
                ...
                for (int i = 0; i < pieces.length; i++) {
                    int nextChar = Integer.parseInt(pieces[i]);
                    ret.add(nextChar);
                }
            }
