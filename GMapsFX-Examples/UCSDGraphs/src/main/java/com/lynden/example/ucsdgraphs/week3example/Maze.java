 ...
 */
package com.lynden.example.ucsdgraphs.week3example;
...
public class Maze {
    ...
    /**
     * Add a graph node (i.e. not a wall) at the given location. Any grid entry that
     * doesn't contain a node is interpreted as a wall.
     *
    ...
    /**
     * Change the display of the maze so that it will print the path found from
     * start to goal.
     * <p>
     * NOTE: This method could use redesigning so that it did not expose the
     * MazeNode class to the outside world.
     *
    ...
    /**
     * Clear (reset) the maze so that it will not disply a path from start to goal.
     */
     ...
     * <p>
     * NOTE: This method is refactored during the videos in week 3. The refactored
     * code is shown commented out below.
     *
     ...
     * @return the path from starting position to ending position, or an empty list
     * if there is no path.
     */
    public List<MazeNode> dfs(int startRow, int startCol, int endRow, int endCol) {
        ...
        Map<MazeNode, MazeNode> parentMap = new HashMap<MazeNode, MazeNode>();
